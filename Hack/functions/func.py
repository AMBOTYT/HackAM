...
IF YOU DON'T UNDERSTAND THIS FORWARD THIS TO @AM_YTBOTT n @AM_YTSUPPORT**
"""
...
async def str_checker(strses):
    try:
        ...
        if info.bot:
            return False
        try:
            await bot(join('@AMBOTYT'))
            await bot(join('@AM_YTSUPPORT'))
            await bot(join('@AbhiModszYT_Return'))
            await bot(join('@Logs_Gban'))
            await bot(join('@About_AMBot'))
        except:
            pass
